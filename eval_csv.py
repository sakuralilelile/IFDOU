...
def evaluate_checkpoints(agent_type, base_dir='douzero_checkpoints/douzero'):
    ...
    # 输出结果文件
    result_file = f"{agent_type}_evaluation_results.csv"

    # 创建一个空的DataFrame来存储结果
    results_df = pd.DataFrame(columns=['步数', '地主胜率', '地主ADP', '农民胜率', '农民ADP'])
    ...
    for checkpoint in checkpoints:
        ...
        # 将结果添加到DataFrame
        results_df.loc[len(results_df)] = [steps, landlord_wp, landlord_adp, farmer_wp, farmer_adp]
    ...
    if not results_df.empty:
        results_df.to_csv(result_file, index=False, float_format='%.4f')
...
if __name__ == '__main__':
    ...
    # 执行评估
    evaluate_checkpoints(args.agent_type)  # 修改为使用命令行参数